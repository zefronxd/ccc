...
from ZEFRONMUSIC.utils import PURVIBin, get_channeplayCB
from ZEFRONMUSIC.utils.database import get_cmode, is_active_chat
...
async def get_queue(client, message: Message, _):
    ...
    await message.reply_photo(IMAGE, caption=cap)
...
async def queued_tracks(client, CallbackQuery: CallbackQuery, _):
    ...
    if "Queued" in msg:
        if len(msg) < 700:
            ...
            return await CallbackQuery.edit_message_text(msg)
        if "✨" in msg:
            msg = msg.replace("✨", "")
